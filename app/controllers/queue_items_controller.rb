class QueueItemsController < ApplicationController
  ...
  def update_queue
    begin
      ...
      flash[:error] = "Invalid List Order Input"
    end
  ...
  def update_queue_items
    ActiveRecord::Base.transaction do
      params[:queue_items].each do |queue_item_data|
        ...
        queue_item.update_attributes!(position: queue_item_data["position"], rate: queue_item_data["rate"]) if queue_item.user == current_user
      end
    end
  end
end